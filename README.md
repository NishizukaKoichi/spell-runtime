...
- `spell license inspect <name>` prints `issuer`, `mode`, `currency`, `max_amount`, `window`, and `revoked`.
...
`spell license list` prints entitlement summary columns (`issuer`, `mode`, `currency`, `max_amount`, `expires_at`, `revoked`) and never prints raw tokens.
